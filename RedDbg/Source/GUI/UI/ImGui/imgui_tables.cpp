...
void ImGui::TableNextRow(ImGuiTableRowFlags row_flags, float row_min_height, unsigned long long* ItemCount)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
...
//extern unsigned long long ItemCount;
void ImGui::TableBeginRow(ImGuiTable* table, unsigned long long* asdasd)
{
    ImGuiWindow* window = table->InnerWindow;
    IM_ASSERT(!table->IsInsideRow);

    // New row
   // if (asdasd != nullptr) { ItemCount++; }
    table->CurrentRow++;
    table->CurrentColumn = -1;
