...
struct MemoryEditor
{
    ...
    static std::string hexStr(unsigned char data, int len)
    {
        std::string s(len * 2, ' ');
        for (int i = 0; i < len; ++i) {
            s[2 * i] = hexmap[(data & 0xF0) >> 4];
            s[2 * i + 1] = hexmap[data & 0x0F];
        }
        return s;
    }

    bool IsMouseHoveringCurrentTableCell(float rowHeight, ImVec2 cellCoordinates, Sizes& _Sizes, bool Text = false)
    {
        //ImVec2 cellMin = ImGui::GetCursorScreenPos();
        if (Text)
        {
            cellCoordinates.y = ImGui::GetCursorScreenPos().y;
            ImVec2 cellMax = ImVec2(cellCoordinates.x + _Sizes.GlyphWidth, cellCoordinates.y);
            return ImGui::IsMouseHoveringRect({ cellCoordinates.x, cellCoordinates.y - rowHeight }, cellMax);
        }
        else
        {
            cellCoordinates.y = ImGui::GetCursorScreenPos().y;
            ImVec2 cellMax = ImVec2(cellCoordinates.x + _Sizes.HexCellWidth, cellCoordinates.y);
            return ImGui::IsMouseHoveringRect({ cellCoordinates.x, cellCoordinates.y - rowHeight }, cellMax);
        }
    }

    inline static std::vector<size_t> Selections; 

    typedef struct _DrawSelectionFrame
    {
    public:
        uint32_t x;
        uint32_t y;
    ...
    } DrawSelectionFrame, *PDrawSelectionFrame;

    void drawSelectionFrame(DrawSelectionFrame DrawSelectionFrame) const
    {
        auto drawList = ImGui::GetWindowDrawList();

        // Draw background color
        drawList->AddRectFilled(
            DrawSelectionFrame.cellPos, 
            { DrawSelectionFrame.cellPos.x + DrawSelectionFrame.cellSize.x, DrawSelectionFrame.cellPos.y + DrawSelectionFrame.cellSize.y },
            DrawSelectionFrame.backgroundColor);

        const ImU32 SelectionFrameColor = ImGui::GetColorU32(ImGuiCol_Text);

        // Draw vertical line at the left of first byte and the start of the line
        if (DrawSelectionFrame.x == 0 || DrawSelectionFrame.byteAddress == DrawSelectionFrame.StartAddress)
        {
            drawList->AddLine(
                DrawSelectionFrame.cellPos,
                { DrawSelectionFrame.cellPos.x + ImVec2(0, DrawSelectionFrame.cellSize.y).x, DrawSelectionFrame.cellPos.y + ImVec2(0, DrawSelectionFrame.cellSize.y).y },
                ImColor(SelectionFrameColor),
                1);
        }

        // Draw vertical line at the right of the last byte and the end of the line
        if (DrawSelectionFrame.x == uint16_t((16 / DrawSelectionFrame.bytesPerCell) - 1) || (DrawSelectionFrame.byteAddress + DrawSelectionFrame.bytesPerCell) > DrawSelectionFrame.EndAddress ||
            DrawSelectionFrame.byteAddress == DrawSelectionFrame.EndAddress)
        {
            drawList->AddLine(
                { DrawSelectionFrame.cellPos.x + ImVec2(DrawSelectionFrame.cellSize.x, -1).x, DrawSelectionFrame.cellPos.y + ImVec2(DrawSelectionFrame.cellSize.x, -1).y },
                { DrawSelectionFrame.cellPos.x + DrawSelectionFrame.cellSize.x, DrawSelectionFrame.cellPos.y + DrawSelectionFrame.cellSize.y },
                ImColor(SelectionFrameColor),
                1);
        }
        ...
        if (DrawSelectionFrame.y == 0 || (DrawSelectionFrame.byteAddress - 16) < DrawSelectionFrame.StartAddress)
        {
            drawList->AddLine(
                DrawSelectionFrame.cellPos,
                { DrawSelectionFrame.cellPos.x + ImVec2(DrawSelectionFrame.cellSize.x + 1, 0).x, DrawSelectionFrame.cellPos.y + ImVec2(DrawSelectionFrame.cellSize.x + 1, 0).y },
                ImColor(SelectionFrameColor),
                1);
        }

        // Draw horizontal line at the bottom of the bytes
        if ((DrawSelectionFrame.byteAddress + 16) > DrawSelectionFrame.EndAddress)
        {
            drawList->AddLine(
                { DrawSelectionFrame.cellPos.x + ImVec2(0, DrawSelectionFrame.cellSize.y).x, DrawSelectionFrame.cellPos.y + ImVec2(0, DrawSelectionFrame.cellSize.y).y },
                { DrawSelectionFrame.cellPos.x + DrawSelectionFrame.cellSize.x + ImVec2(1, 0).x, DrawSelectionFrame.cellPos.y + DrawSelectionFrame.cellSize.y + ImVec2(1, 0).y },
                ImColor(SelectionFrameColor),
                1);
        }
    }

    static void HandleCellSelection(size_t addr, bool& IsCtrlPressed, bool& IsInverse, bool& IsShiftPressed, size_t& selectionStart, size_t& selectionEnd)
    {
        if (ImGui::IsMouseDragging(ImGuiMouseButton_Left))
        {
            if (ImGui::GetIO().KeyCtrl)
            {
                for (size_t CurrAddr = selectionStart; CurrAddr < selectionEnd; ++CurrAddr)
                {
                    if (std::find(Selections.begin(), Selections.end(), CurrAddr) == Selections.end())
                    {
                        Selections.push_back(CurrAddr);
                    }
                }

                if (std::find(Selections.begin(), Selections.end(), addr) == Selections.end())
                {
            ...
            //}
            {
                // Если текущий адрес меньше начала выделения, обменяем их местами
    ...
    void DrawContents(void* mem_data_void, size_t mem_size, size_t base_display_addr = 0x0000)
    {
        ...
        static ImVec2 CharSize = ImGui::CalcTextSize("FF");
        ...
        while (clipper.Step())
        {
            for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible lines
            {
                ...
                for (int n = 0; n < Cols && addr < mem_size; n++, addr++)
                {
                    ...
                    ImVec2 cellCoordinates;
                    cellCoordinates.x = byte_pos_x + CharSize.x;
                    cellCoordinates.y = ImGui::GetCursorPosY();
                    ...
                    else
                    {
                        ...
                        if (IsCtrlPressed)
                        {
                            ...
                            for (auto& Addr : Selections)
                            {
                                if (Addr == addr)
                                {
                                    DrawSelectionFrame objDrawSelectionFrame;
                                    objDrawSelectionFrame.x = n;
                                    objDrawSelectionFrame.y = line_i;
                                    objDrawSelectionFrame.StartAddress = Addr;
                                    objDrawSelectionFrame.EndAddress = Addr;
                                    objDrawSelectionFrame.byteAddress = Addr;
                                    objDrawSelectionFrame.bytesPerCell = 1;
                                    objDrawSelectionFrame.cellPos = { cellCoordinates.x, ImGui::GetCursorScreenPos().y - CharSize.y };
                                    objDrawSelectionFrame.cellSize = { s.HexCellWidth, CharSize.y };
                                    objDrawSelectionFrame.backgroundColor = ImColor(39, 0, 255, 50);
                                    drawSelectionFrame(objDrawSelectionFrame);
                                }
                            }
                        }
                        ...
                        {
                            if (IsInverse == true) { std::swap(selectionStart, selectionEnd); }
                            if (selectionEnd <= mem_size - 1)
                            {
                                if ((selectionEnd - selectionStart))
                                {
                                    for (size_t CurrAddr = selectionStart; CurrAddr <= selectionEnd; ++CurrAddr)
                                    {
                                        if (CurrAddr == addr)
                                        {
                                            ...
                                            objDrawSelectionFrame.x = n;
                                            objDrawSelectionFrame.y = line_i;
                                            objDrawSelectionFrame.StartAddress = selectionStart;
                                            objDrawSelectionFrame.EndAddress = selectionEnd;
                                            objDrawSelectionFrame.byteAddress = CurrAddr;
                                            objDrawSelectionFrame.bytesPerCell = 1;
                                            objDrawSelectionFrame.cellPos = { cellCoordinates.x, ImGui::GetCursorScreenPos().y - CharSize.y };
                                            objDrawSelectionFrame.cellSize = { s.HexCellWidth, CharSize.y };
                                            objDrawSelectionFrame.backgroundColor = ImColor(39, 0, 255, 50);
                                ...
                                else if(selectionStart == addr)
                                {
                                    ...
                                    objDrawSelectionFrame.StartAddress = selectionStart;
                                    objDrawSelectionFrame.EndAddress = selectionEnd;
                                    objDrawSelectionFrame.byteAddress = selectionStart;
                                    objDrawSelectionFrame.bytesPerCell = 1;
                                    objDrawSelectionFrame.cellPos = { cellCoordinates.x, ImGui::GetCursorScreenPos().y - CharSize.y };
                                    objDrawSelectionFrame.cellSize = { s.HexCellWidth, CharSize.y };
                                    objDrawSelectionFrame.backgroundColor = ImColor(39, 0, 255, 50);
                            ...
                            if (IsInverse == true) { std::swap(selectionEnd, selectionStart); }
                        }

                        if (!ReadOnly && ImGui::IsItemHovered() && ImGui::IsWindowHovered())
                        {
                            HandleCellSelection(addr, IsCtrlPressed, IsInverse, IsShiftPressed, selectionStart, selectionEnd);
                        }
                        else if (ImGui::GetIO().KeyCtrl && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_A)))
                        {
                            selectionStart = 0;
                            selectionEnd = mem_size - 1;
                        }

                        //Addr for edit
                        if (!ReadOnly && ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left))//ImGui::IsMouseClicked(ImGuiMouseButton_Left))
                        {
                            DataEditingTakeFocus = true;
                            data_editing_addr_next = addr;
                        }

                        // Проверяем, нажата ли комбинация CTRL+C
                        if (ImGui::GetIO().KeyCtrl && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_C)))
                        {
                            ...
                            if (Selections.empty())
                            {
                                for (size_t CurrAddr = selectionStart; CurrAddr < selectionEnd; ++CurrAddr)
                                {
                                    selectedData += hexStr(mem_data[CurrAddr], sizeof(ImU8));
                                }
                                selectedData += "\n";
                            }
                            ImGui::SetClipboardText(selectedData.c_str());
                        }
                    }
                }
                ...
                if (OptShowAscii)
                {
                    ...
                    ImGui::PushID(line_i);
                    if (ImGui::InvisibleButton("ascii", ImVec2(s.PosAsciiEnd - s.PosAsciiStart, s.LineHeight), ImGuiButtonFlagsPrivate_::ImGuiButtonFlags_PressedOnDoubleClick))
                    {
                    ...
                    for (int n = 0; n < Cols && addr < mem_size; n++, addr++)
                    {
                        ...
                        if (IsCtrlPressed)
                        {
                            ...
                            for (auto& Addr : Selections)
                            {
                                if (Addr == addr)
                                {
                                    ...
                                    objDrawSelectionFrame.byteAddress = Addr;
                                    objDrawSelectionFrame.bytesPerCell = 1;
                                    objDrawSelectionFrame.cellPos = { pos.x, ImGui::GetCursorScreenPos().y - CharSize.y };
                                    objDrawSelectionFrame.cellSize = { s.GlyphWidth, CharSize.y };
                                    objDrawSelectionFrame.backgroundColor = ImColor(39, 0, 255, 50);
                                    drawSelectionFrame(objDrawSelectionFrame);
                                }
                            }
                        ...
                        if (!ReadOnly && IsMouseHoveringCurrentTableCell(s.LineHeight, ImVec2(pos.x, pos.y - s.LineHeight), s)/*ImGui::IsItemHovered()*/ && ImGui::IsWindowHovered())
                        {
                            HandleCellSelection(addr + (size_t)((ImGui::GetIO().MousePos.x - pos.x) / s.GlyphWidth), IsCtrlPressed, IsInverse, IsShiftPressed, selectionStart, selectionEnd);
                        }
